...
declare
  ...
  v_seller_id uuid := coalesce(p_seller_profile_id, v_actor_id);
begin
  ...
  if p_sale_type = 'buy_now' and p_buy_now_price_pence is null then
    raise exception 'Buy-now listings require a price.';
  end if;
  ...
  if p_sale_type = 'auction' and (
    ...
    raise exception 'Auction listings require a starting bid, start time, and end time.';
  end if;

  if p_sale_type = 'auction' and p_auction_ends_at <= p_auction_starts_at then
    raise exception 'Auction end time must be after the start time.';
  end if;
  ...
  if v_seller_id is not null and not exists (
    ...
    raise exception 'Selected seller profile was not found.';
  end if;
...
begin
  ...
  return query
  select v_started, v_ended, v_settled;
end;
$$;
...
begin
  ...
  perform public.sync_auction_states();
  ...
  if v_profile.role <> 'admin' and v_profile.bidder_status <> 'approved' then
    raise exception 'Your account is still waiting for bidder approval.';
  end if;
  ...
  if v_auction.current_bid_id is null then
    v_minimum_bid := coalesce(v_listing.starting_bid_pence, 0);
  else
    select bidder_id
    into v_previous_high_bidder
    from public.bids
    where id = v_auction.current_bid_id;

    v_minimum_bid := coalesce(v_auction.current_bid_pence, 0) + coalesce(v_listing.bid_increment_pence, 5000);
  end if;
